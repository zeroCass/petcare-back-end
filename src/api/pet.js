...
router.get("/:idPet", (req, res) => {
	...
	db.query(query, [idPet], (err, result) => {
		...
		if (result.length === 0) return res.status(404).send("Pet nao encontrado");
...
router.post("/", (req, res) => {
	...
	db.query(
		query,
		[pet.name, pet.sex, pet.species, pet.breed, pet.size, pet.weight, pet.birthDate, pet.cpfTutor, arrayBufferImg],
		(err, result) => {
			if (err) {
...
router.put("/", async (req, res) => {
	...
	db.query(
		query,
		[pet.idPet, pet.name, pet.sex, pet.species, pet.breed, pet.size, pet.weight, pet.birthDate, arrayBufferImg],
		(err, result) => {
			if (err) {
